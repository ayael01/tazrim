...
export default function BankImportHistoryCard({ batches, onRollback, onView }) {
  const [status, setStatus] = useState("");
  const [error, setError] = useState("");

  async function handleRollback(batchId) {
    if (!window.confirm("Delete this import batch and its activities?")) {
      return;
    }
    setStatus("Rolling back...");
    setError("");
    try {
      const response = await fetch(`${API_BASE}/bank/imports/${batchId}`, {
        method: "DELETE",
      });
      if (!response.ok) {
